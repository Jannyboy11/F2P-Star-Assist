package com.janboerman.f2pstarassist.generator
...
private def checkDirs(): Option[(os.Path, os.Path, os.Path, os.Path)] = {
    val rootDir = os.pwd
    if (!rootDir.endsWith(os.RelPath("F2P-Star-Assist"))) {
        println("Please run this script from root directory (F2P-Star-Assist)")
        return None
    }
...
private def buildDotGradle(runeliteVersion: String, lombokVersion: String, junitVersion: String, f2pStarAssistVersion: String, f2pStarAssistGroup: String): String =
   s"""plugins {
   |	id 'java'
   |}
   |
   ...
   |}
   |
   |group = '${f2pStarAssistGroup}'
   |version = '${f2pStarAssistVersion}'
